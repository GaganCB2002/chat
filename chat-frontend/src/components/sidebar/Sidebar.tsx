...
import {
  ...
  LogOut, PanelLeftClose, PanelLeft, ChevronDown, MoreVertical, Copy, Archive, Trash2, Star, Download, ChevronUp, User, Mail, Calendar, Shield
} from 'lucide-react';
  ...
  const content = (
    <div className="flex flex-col h-full bg-surface-secondary ">
      ...
      <div className="border-t border-border">
        <button onClick={() => setMenuOpen(!menuOpen)} className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-text-secondary hover:bg-black/5 dark:hover:bg-white/5 transition-all">
          {menuOpen ? <ChevronDown className="w-3.5 h-3.5" /> : <ChevronUp className="w-3.5 h-3.5" />}
          <span>Menu</span>
        </button>
        {menuOpen && (
...
function ProfileBadge() {
  ...
  return (
    <div className="relative px-1 pb-1">
      <button onClick={() => isAuthenticated ? setShowProfile(!showProfile) : setShowAuthModal(true, 'login')} className="w-full flex items-center gap-3 pt-2 mt-1 border-t border-border hover:bg-black/5 dark:hover:bg-white/5 rounded-lg px-2 py-1.5 transition-all">
        <div className="w-8 h-8 rounded-full bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 flex items-center justify-center font-bold text-sm flex-shrink-0">
          {initials}
        </div>
        <div className="flex-1 min-w-0 text-left">
          <p className="text-sm font-medium text-text truncate">{displayName}</p>
          <p className="text-[11px] text-text-tertiary truncate">{planLabel}</p>
        </div>
        <ChevronUp className={cn('w-3.5 h-3.5 text-text-tertiary transition-transform', showProfile && 'rotate-180')} />
      </button>
      {showProfile && isAuthenticated && user && (
        <div className="mx-2 mb-1 p-3 rounded-lg bg-surface-tertiary border border-border space-y-2">
          ...
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="text-text-secondary">Free plan</span>
          </div>
        </div>
      )}
    </div>
